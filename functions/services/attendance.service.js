...
class AttendanceService {
  ...
  async createAttendanceRecord(attendanceData, studioOwnerId) {
    ...
    // For classes: use class-specific credits first, then general credits
    // For workshops/events: use general credits only
    const classId = attendanceData.classId || null;
    let availableCredits;
    
    if (classId) {
      // For classes: check class-specific credits first
      availableCredits = await creditTrackingService.getAvailableCredits(
          attendanceData.studentId,
          studioOwnerId,
          classId
      );
      
      // If no class-specific credits, check general credits
      if (availableCredits < 1) {
        availableCredits = await creditTrackingService.getAvailableCredits(
            attendanceData.studentId,
            studioOwnerId,
            null // General credits
        );
      }
    } else {
      // For workshops/events: use general credits only
    ...
    // For classes: try class-specific credits first, then general credits
    // For workshops/events: use general credits only
    let creditUsedId;
    try {
      if (classId) {
        // Try class-specific credits first
        try {
        ...
        } catch (classCreditError) {
          // If no class-specific credits, use general credits
          creditUsedId = await creditTrackingService.useCredit(
              attendanceData.studentId,
              studioOwnerId,
              null // General credits

...
class CreditTrackingService {
  ...
  async addCredits(studentId, studioOwnerId, credits, expirationDays, packageId = null, classId = null) {
    ...
    const creditEntry = {
      ...
      packageId: packageId,
      classId: classId || null, // Store classId (null for general credits)
   ...
   * @param {string} studentId - Student document ID
   * @param {string} studioOwnerId - Studio owner document ID
   * @param {string} classId - Optional class ID to filter for class-specific credits (null for general credits only)
  ...
  async getAvailableCredits(studentId, studioOwnerId, classId = null) {
    ...
    if (classId) {
      // Get class-specific credits for this class
      ...
      // Get general credits (classId is null) or all credits if classId not specified
      snapshot = await creditsRef
          .where("studioOwnerId", "==", studioOwnerId)
          .where("expirationDate", ">", now)
          .get();
    ...
    snapshot.forEach((doc) => {
      const data = doc.data();
      // If classId was specified, only count credits where classId matches
      // If classId was null, count all credits (both general and class-specific)
   ...
   * @param {string} studentId - Student document ID
   * @param {string} studioOwnerId - Studio owner document ID
   * @param {string} classId - Optional class ID to use class-specific credits (null for general credits)
  ...
  async useCredit(studentId, studioOwnerId, classId = null) {
    ...
    if (classId) {
      // Get class-specific credits for this class, ordered by purchase date (oldest first)
      snapshot = await creditsRef
          .where("studioOwnerId", "==", studioOwnerId)
          .where("classId", "==", classId)
          .where("expirationDate", ">", now)
      ...
      // Get general credits (classId is null), ordered by purchase date (oldest first)
      snapshot = await creditsRef
          .where("studioOwnerId", "==", studioOwnerId)
          .where("classId", "==", null)
          .where("expirationDate", ">", now)

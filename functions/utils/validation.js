...
function validateCreatePackagePayload(payload) {
  ...
  if (payload.isRecurring !== undefined && payload.isRecurring !== null) {
    if (typeof payload.isRecurring !== "boolean") {
      ...
      // If recurring is enabled, validate billing frequency and interval
      if (payload.billingFrequency === undefined || payload.billingFrequency === null) {
        ...
        // Validate billing frequency type
        const validFrequencies = ['monthly', 'weekly', 'daily'];
        const isNumber = typeof payload.billingFrequency === 'number' && !isNaN(payload.billingFrequency);
        const isString = typeof payload.billingFrequency === 'string' && validFrequencies.includes(payload.billingFrequency);
        if (!isNumber && !isString) {
          errors.push({field: "billingFrequency", message: "Billing frequency must be 'monthly', 'weekly', 'daily', or a number (days)"});
        } else if (isNumber && payload.billingFrequency < 1) {
      ...
      } else if (typeof payload.billingInterval !== "number" || isNaN(payload.billingInterval) || payload.billingInterval < 1) {
        errors.push({field: "billingInterval", message: "Billing interval must be a positive number"});
      }

      // Validate subscription duration (optional - if provided, must be positive integer)
      if (payload.subscriptionDuration !== undefined && payload.subscriptionDuration !== null) {
        if (typeof payload.subscriptionDuration !== "number" || isNaN(payload.subscriptionDuration) || payload.subscriptionDuration < 1) {
...
function validateUpdatePackagePayload(payload) {
  ...
  // Recurring payment validation (optional for update)
  if (payload.isRecurring !== undefined && payload.isRecurring !== null) {
    if (typeof payload.isRecurring !== "boolean") {
      errors.push({field: "isRecurring", message: "Is recurring must be a boolean"});
    } else if (payload.isRecurring) {
